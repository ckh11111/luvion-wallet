...
export default function LuvionWalletV3() {
  const [activeTab, setActiveTab] = useState('assets'); // assets or altar
  const [clickedIndex, setClickedIndex] = useState<number | null>(null);

  // 点击资产条目的动态效果
  const handleItemClick = (index: number) => {
    setClickedIndex(index);
    setTimeout(() => setClickedIndex(null), 150); // 150ms后恢复
  };
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            <>
              ...
              <div className="bg-[#222E46]/95 border-[8px] border-[#181D2A] outline outline-[4px] outline-[#93A0B4] p-8 shadow-[10px_10px_0_rgba(0,0,0,0.4)]">
                <div className="flex justify-between items-center">
                  <div>
                    <p className="text-[#A9B2C3] text-sm mb-1 font-arcade uppercase">总资产 (TOTAL GOLD COINS)</p>
                    <h2 className="text-6xl text-[#FFD700] drop-shadow-[4px_4px_0_#000] font-arcade">$1,234.56</h2>
                  </div>

                  {/* 恢复之前的配色：金黄加密，灰白接收发送 */}
                  <div className="flex gap-2">
                    <button className="bg-[#FFD700] text-black px-4 py-4 border-4 border-[#B8860B] shadow-[inset_-4px_-4px_0_rgba(0,0,0,0.3)] hover:brightness-110 active:translate-y-1 font-arcade text-[10px]">
                      <PixelText text="初始化加密" />
                    </button>
                    <button className="bg-[#E5E7EB] text-black px-4 py-4 border-4 border-[#9CA3AF] shadow-[inset_-4px_-4px_0_rgba(0,0,0,0.2)] hover:brightness-110 active:translate-y-1 font-arcade text-[10px]">
                      <PixelText text="接收 (+)" />
                    </button>
                    <button className="bg-[#E5E7EB] text-black px-4 py-4 border-4 border-[#9CA3AF] shadow-[inset_-4px_-4px_0_rgba(0,0,0,0.2)] hover:brightness-110 active:translate-y-1 font-arcade text-[10px]">
                      <PixelText text="发送 (↗)" />
          ...
          {activeTab === 'magic' && (
            <div className="bg-[#222E46]/95 border-[8px] border-[#181D2A] outline outline-[4px] outline-[#93A0B4] p-8 animate-fade-in">
              ...
              <div className="mt-8 p-4 bg-black/40 border-4 border-[#181D2A] text-[#A9B2C3] text-xs leading-relaxed italic">
                <PixelText text="* 只要网络中存在至少 12 个活跃分片，资产即可实现量子级自愈。当前状态：安全。" />
              </div>
            </div>
          )}
        </div>
